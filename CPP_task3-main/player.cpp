...
{
    if (isMyTurn_ == false || this->isMyTurn_ == false)
    {
...
void player::placeCity(board &gameBoard, int vertexId)
{
    ...
    victoryPoints_++;
    GAME_IS_OVER();//check if the game is over because of the victory points is updated
...
void player::useDevelopmentCardMonopol(resourceTypes resource, catan &game)
{
...
void player::useDevelopmentCardRoadBuilding(board &gameBoard, int edgeId1, int edgeId2, catan &game)
{
...
void player::useDevelopmentCardYearOfPlenty(resourceTypes resource1, resourceTypes resource2, catan &game)
{
...
void player::useDevelopmentCardVictoryPoint(catan &game)
{
...
void player::updateLargestArmy()
{
    if (NumOfKnights_ >= 3)
    {
        ...
        {
            if (NumOfKnights_ > playerWithLargestArmy_->NumOfKnights_)
            {
                if (playerWithLargestArmy_->victoryPoints_ != 0 && playerWithLargestArmy_->victoryPoints_ != 1)
                {
...
void player::buyDevelopmentCardTest(DevelopmentCard card)
{
...
void player::endTurn(catan &game)
{
    ...
    std::cout << "Player " << name_ << " ended his/her turn.\n"
              << std::endl;
    if (this == players_[2])
    {
        players_[2]->setIsMyTurn(false);
        players_[2]->isMyTurn_ = false;
        players_[1]->setIsMyTurn(false);
        players_[1]->isMyTurn_ = false;
        players_[0]->setIsMyTurn(true);
        players_[0]->isMyTurn_ = true;
        game.setCurrentPlayer(players_[0]);
        std::cout << "Player " << players_[0]->getName() << " it is your turn." << std::endl;
    }
    if (this == players_[0])
    {
        players_[0]->setIsMyTurn(false);
        players_[0]->isMyTurn_ = false;
        players_[2]->setIsMyTurn(false);
        players_[2]->isMyTurn_ = false;
        players_[1]->setIsMyTurn(true);
        players_[1]->isMyTurn_ = true;
        game.setCurrentPlayer(players_[1]);
        std::cout << "Player " << players_[1]->getName() << " it is your turn." << std::endl;
    }
    if (this == players_[1])
    {
        players_[1]->setIsMyTurn(false);
        ...
        players_[0]->setIsMyTurn(false);
        players_[0]->isMyTurn_ = false;
        players_[2]->setIsMyTurn(true);
        players_[2]->isMyTurn_ = true;
        game.setCurrentPlayer(players_[2]);
        std::cout << "Player " << players_[2]->getName() << " it is your turn." << std::endl;
    }
}
...
void player::ForTestAddDevelopmentCard(DevelopmentCard card)
{
    developmentCards_[card]++;
}
int player::getNumOfKnights()
{
    return NumOfKnights_;
}
void player::setNumOfKnights(int num)
{
    NumOfKnights_ = num;
}
void player::setVictoryPoints(int num){
    victoryPoints_ = num;
}
